    ...
    <ScrollView contentContainerStyle={{}} className="bg-white grow">
      <View className="p-4 pt-12">
        <View className="items-center mb-6">
          ...
          <Text className="text-3xl font-bold text-gray-800 text-center">{kebab.name}</Text>
          <Text className="text-lg text-gray-600 text-center">{kebab.address}</Text>
        </View>
        ...
        {kebab.opening_hours && Object.keys(kebab.opening_hours).length > 0 && (
          <View className="bg-custom-beige rounded-lg p-4 shadow-lg mb-4">
            <Text className="text-xl font-bold text-gray-800 mb-2">
              <Ionicons name="time" size={20} color={iconColor} /> Godziny otwarcia:
            </Text>
        ...
        {kebab.sauces && kebab.sauces.length > 0 && (
          <View className="bg-custom-beige rounded-lg p-4 shadow-lg mb-4">
            <Text className="text-xl font-bold text-gray-800 mb-2">
              <Ionicons name="flask-outline" size={20} color={iconColor} /> Dostępne sosy:
            </Text>
        ...
        {kebab.meats && kebab.meats.length > 0 && (
          <View className="bg-custom-beige rounded-lg p-4 shadow-lg mb-4">
            <Text className="text-xl font-bold text-gray-800 mb-2">
              <Ionicons name="fast-food-outline" size={20} color={iconColor} /> Rodzaje mięs:
            </Text>
        ...
        {kebab.status && (
          <View className="bg-custom-beige rounded-lg p-4 shadow-lg mb-4">
            <Text className="text-xl font-bold text-gray-800 mb-2">
              <Ionicons name="checkmark-circle" size={20} color={iconColor} /> Status:
            </Text>
            <Text className={`text-lg ${kebab.status === 'exists' ? 'text-green-600' : 'text-red-600'}`}>
              {statusTranslation}
            </Text>
          </View>
        )}
        ...
        <View className="bg-custom-beige rounded-lg p-4 shadow-lg mb-4">
          {kebab.is_crafted !== undefined && (
            <Text className="text-gray-700">
              <Ionicons name="star" size={16} color={iconColor} /> Kraftowy: {kebab.is_crafted ? 'Tak' : 'Nie'}
            </Text>
          )}
          {kebab.is_premises !== undefined && (
            <Text className="text-gray-700 mt-2">
              <Ionicons name="home" size={16} color={iconColor} /> Lokal: {kebab.is_premises ? 'Tak' : 'Nie'}
            </Text>
          )}
          {kebab.is_chainstore !== undefined && (
            <Text className="text-gray-700 mt-2">
              <Ionicons name="grid-outline" size={16} color={iconColor} /> Sieć: {kebab.is_chainstore ? 'Tak' : 'Nie'}
            </Text>
          )}
        </View>
        ...
        {kebab.ordering_options && kebab.ordering_options.length > 0 && (
          <View className="bg-custom-beige rounded-lg p-4 shadow-lg mb-4">
            <Text className="text-xl font-bold text-gray-800 mb-2">
              <Ionicons name="cart" size={20} color={iconColor} /> Opcje zamówienia:
            </Text>
            <Text className="text-gray-700">{kebab.ordering_options.join(', ')}</Text>
          </View>
        )}
        ...
        <View className="mt-4">
          {kebab.pages?.['pyszne.pl'] && (
            <TouchableOpacity
              ...
              <Text className="text-white text-center font-bold">Menu na Pyszne.pl</Text>
            </TouchableOpacity>
          )}
          {kebab.pages?.glovo && (
            <TouchableOpacity
              ...
              <Text className="text-white text-center font-bold">Zamów przez Glovo</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
